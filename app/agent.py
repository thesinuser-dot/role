...
class InstagramAgent:
    ...
    def _process_task(
        ...
        if not skip_vision:
            ...
            try:
                vision_ok, vision_reason = self.vision.evaluate(screenshot, views, likes)
            except Exception as exc:
                self.log.error(
        ...
        try:
            # Pass preserve_file=True so notifier does NOT delete the video —
            # we need it on disk for the TikTok upload that follows.
            sent = self.notifier.send_qualified_reel(
                video_path, reel_url, views, likes,
                preserve_file=self.tiktok.enabled,
            )
        except Exception as exc:
            self.log.error(
        ...
        if sent:
            ...
            if self.tiktok.enabled and video_path.exists():
                ...
                if tiktok_ok:
                    ...
                    self.log.warning(
                        f"[{reel_id}] ⚠️ TikTok post failed — "
                        "Telegram delivery was still marked successful."
                    )
                # Always clean up the local file now that TikTok attempt is done.
                try:
                    video_path.unlink(missing_ok=True)
                    self.log.info(f"[{reel_id}] Local file deleted after TikTok attempt.")
                except OSError as exc:
                    self.log.warning(f"[{reel_id}] Could not delete local file: {exc}")
        else:
